...
public class FrontController extends HttpServlet {
	...
	public void init(ServletConfig config) throws ServletException {
		...
		map.put("/orderPage.do", new orderControl());						// 주문 페이지
		map.put("/searchProduct.do", new searchProductControl());			// 검색 결과
		map.put("/logOut.do", new LogoutControl());							// 로그아웃
		map.put("/cart.do", new cartControl());								// 메인에서 장바구니 담기
		map.put("/addZzim.do", new addZzimControl());						// 찜 추가 기능
		map.put("/selectCoupon.do", new selectCouponControl());				// 쿠폰 조회 기능
		map.put("/addOrder.do", new addOrderControl());						// 주문 정보 저장 기능
		map.put("/removeCart.do", new removeCartControl());					// 장바구니 삭제
		map.put("/adminMain.do", new adminMainControl());					// 관리자 페이지 메인
		map.put("/orderManage.do", new orderManageControl());				// 주문 관리페이지
		map.put("/addPayment.do", new addPaymentControl());					// 지불 정보 입력
		map.put("/addOrderProduct.do", new addOrderProductControl());		// 주문 상품정보 입력
		map.put("/searchOrder.do", new searchOrderControl());				// 주문 상품정보 입력
		...
		map.put("/findId.do", new FindId());								// 비번 찾기 처리
		map.put("/findIdResult.do", new FindIdResult());					// 아이디 찾기 처리
		...
		map.put("/qaManageForm.do", new QaManagerForm());					// Q&A 게시판 관리 페이지
		map.put("/qaList.do", new QaList());								// Q&A 목록
		map.put("/removeQa.do", new QaRemove());							// Q&A 삭제
		map.put("/updateQa.do", new QaUpdate());							// Q&A 수정
		map.put("/searchQaManage.do", new SearchQaManage());				// QA 게시판 검색 (상품번호 proId 이용)
		map.put("/searchMemberManage.do", new SearchMemberManage());		// 회원 검색

		
		// 박주안
		map.put("/myPage.do",new MyPageControl());							// 마이페이지
		map.put("/zzimList.do", new ZzimListControl());						// zzim 목록 출력
		map.put("/noticeList.do", new NoticeListControl());					// 공지사항 페이지 이동
		...
		map.put("/noticeUpdateForm.do", new NoticeUpdateFormControl());		// 공지사항 수정 페이지 이동
		map.put("/noticeUpdate.do", new NoticeUpdateControl());				// 공지사항 수정 기능
		map.put("/updateInfoForm.do", new UpdateInfoForm());				// 개인정보 수정 페이지
		map.put("/updateInfo.do", new UpdateInfo());						// 개인정보 수정 기능
		map.put("/zzimDelete.do", new ZzimDeleteControl());					// 찜 삭제
		map.put("/orderList.do", new OrderListControl());					// 주문 내역 
		map.put("/myNoticeList.do", new MyNoticeListControl());				// 마이페이지 후기 및 Q&A
	}
